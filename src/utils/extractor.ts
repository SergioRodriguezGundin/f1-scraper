...
const setDriverName = <T extends object>(row: Element, $: CheerioAPI, f1Object: T) => {
  ...
  return driverAlias === 'ZHO'
    ? `${driverLastName} ${driverFirstName} ${driverAlias}`
    : `${driverFirstName} ${driverLastName} ${driverAlias}`;
};

const setPosition = (row: Element, $: CheerioAPI, resultIndex: number) => {
  const position = $(row)
    .find(`td:nth-child(${resultIndex + 1})`)
    .text()
    .replace(/\s+/g, ' ')
    .trim();
